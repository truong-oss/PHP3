            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="tieu_de" name="tieu_de"
                        class="form-control @error('tieu_de') is-invalid @enderror" value="{{ old('tieu_de', $post->tieu_de) }}">
                    @error('tieu_de')
                ...
                <div class="mb-3">
                    ...
                    <textarea id="bai_viet" name="bai_viet" class="form-control @error('bai_viet') is-invalid @enderror" rows="6">{{ old('bai_viet', $post->bai_viet) }}</textarea>
                    @error('bai_viet')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="tac_gia" name="tac_gia"
                        class="form-control @error('tac_gia') is-invalid @enderror" value="{{ old('tac_gia', $post->tac_gia) }}">
                    @error('tac_gia')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div>
                        <div class="form-check form-check-inline">
                            <input type="radio" id="hien_thi" name="trang_thai" value="1" class="form-check-input"
                                {{ old('trang_thai', $post->trang_thai) == '1' ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input type="radio" id="an" name="trang_thai" value="0" class="form-check-input"
                                {{ old('trang_thai', $post->trang_thai) == '0' ? 'checked' : '' }}>
