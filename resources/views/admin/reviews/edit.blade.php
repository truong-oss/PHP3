...
@section('content')
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h5><i class="fas fa-edit"></i> Cập Nhật Đánh Giá</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.reviews.update', $review->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="reviewer_name" name="reviewer_name" class="form-control @error('reviewer_name') is-invalid @enderror"
                ...
                <div class="mb-3">
                    ...
                    <textarea id="comment" name="comment" class="form-control @error('comment') is-invalid @enderror" rows="4">{{ $review->comment }}</textarea>
                    @error('comment')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="post_id" name="post_id" class="form-select @error('post_id') is-invalid @enderror">
                        ...
                        @foreach ($posts as $post)
                            <option value="{{ $post->id }}" {{ $review->post_id == $post->id ? 'selected' : '' }}>
                                {{ $post->tieu_de }}
                            </option>
                        @endforeach
                    </select>
